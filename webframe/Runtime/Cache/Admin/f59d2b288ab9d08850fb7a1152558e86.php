	...
	<body>
		<div id="page-content">
			<div id="main-container container-fluid" style="margin-left:40px;">
				<div id="headshow" >
					<button type="button" class="btn btn-success btn-sm" onclick="add();"><span class="glyphicon glyphicon-plus"></span>添加用户</button>
		...
		$('#table').bootstrapTable({
					...
					method: 'get',
					url: "/html/webframework/webframe/index.php/Admin/User/ajaxIndex",
					//cache: false,
		...
		*/
		function add(){
			...
			var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['添加用户', 'font-size:14px;background:#2b9af6;color:#fff'],
						...
						content: ['/html/webframework/webframe/index.php/Admin/User/add/isAdmin/1', 'yes'],
						btn: ['确定', '取消']
						, yes: function (index) {
						...
						if(user_name=="admin"){
							...
							return;
								...
								$.ajax({
									type: 'post',
									url: '/html/webframework/webframe/index.php/Admin/User/insert',
									data: obj.serialize(),
									cache: false,
		...
		*/
		function edit(user_id,user_account_id){
			...
			var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['修改用户', 'font-size:14px;background:#2b9af6;color:#fff'],
						move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
						area: ['100%', '100%'], //设置弹出框的宽高
						shade: [0.5, '#000'], //配置遮罩层颜色和透明度
						shadeClose: true, //是否允许点击遮罩层关闭弹窗 true /false
						...
						// time:1000,  设置自动关闭窗口时间 1秒=1000；
						shift: 0, //打开效果：0-6 。0放大，1从上到下，2下到上，3左到右放大，4翻滚效果；5渐变；6抖窗口
						content: ['/html/webframework/webframe/index.php/Admin/User/edit/isAdmin/1/user_id/'+user_id+'/user_account_id/'+user_account_id, 'yes'],
						btn: ['确定', '取消']
						, yes: function (index) {
						...
						if(user_name=="admin"){
							...
							return;
								...
								$.ajax({
									type: 'post',
									url: '/html/webframework/webframe/index.php/Admin/User/update',
									data: obj.serialize(),
									cache: false,
									success: function (data) {
										if (data.status) {
		...
		*/
		function delmore(user_id,user_account_id){
			layer.confirm('确定要删除吗？', {
					...
					var actionUrl = "/html/webframework/webframe/index.php/Admin/User/del";
						$.ajax({
						type: 'post',
								url: actionUrl,
		...
		*/
		function editPwd(user_account_id){
			var index = layer.open({
						...
						// time:1000,  设置自动关闭窗口时间 1秒=1000；
						shift: 0, //打开效果：0-6 。0放大，1从上到下，2下到上，3左到右放大，4翻滚效果；5渐变；6抖窗口
						content: ['/html/webframework/webframe/index.php/Admin/User/editPwd/user_account_id/'+user_account_id, 'no'],
						btn: ['确定', '取消']
						, yes: function (index) {
						...
						if(user_pwd==""){
							...
							return;
								...
								$.ajax({
									type: 'post',
									url: '/html/webframework/webframe/index.php/Admin/User/editPwdSave',
									data: obj.serialize(),
									cache: false,
									success: function (data) {
										if (data.status) {
