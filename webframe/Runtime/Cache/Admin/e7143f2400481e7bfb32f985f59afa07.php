...
<html>
	...
	<body>
		<div id="page-content">
			<div id="main-container container-fluid" style="margin-left:40px;">
				<div id="headshow" >
					<button type="button" class="btn btn-success btn-sm" onclick="add();"><span class="glyphicon glyphicon-plus"></span>添加角色</button>					
				</div>
				<div id="divTable">
	...
	$('#table').bootstrapTable({
		...
		function handle(value, row, index) {
				...
				return [
						'</a>',
						'<a class="edit ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="edit('+row.role_id+')" title="编辑">',
						'<i class="glyphicon glyphicon-pencil"></i>',
						'</a>',
						'&nbsp;&nbsp;',
						'<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="delmore('+row.role_id+')" title="删除">',
						'<i class="glyphicon glyphicon-trash"></i>',
						'</a>',
						'&nbsp;&nbsp;',
						'<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="accessDis('+row.role_id+')" title="权限分配">',
						'<i class="glyphicon glyphicon-wrench"></i>',
						'</a>',
						'&nbsp;&nbsp;',
						'<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="menuDis('+row.role_id+')" title="菜单分配">',
	...
	*/
	function add(){
		var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['添加角色', 'font-size:14px;background:#2b9af6;color:#fff'],
						move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
						area: ['400px', '330px'], //设置弹出框的宽高
	...
	*/
	function edit(role_id){
		var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['修改角色', 'font-size:14px;background:#2b9af6;color:#fff'],
						move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
	...
	权限分配操作
	*/
	function accessDis(role_id){
		var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['权限分配', 'font-size:14px;background:#2b9af6;color:#fff'],
						move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
	...
	*/
	...
	/*function menuDis(role_id){
		var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['菜单分配', 'font-size:14px;background:#2b9af6;color:#fff'],
						move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
						area: ['100%', '100%'], //设置弹出框的宽高
						shade: [0.5, '#000'], //配置遮罩层颜色和透明度
						shadeClose: true, //是否允许点击遮罩层关闭弹窗 true /false
