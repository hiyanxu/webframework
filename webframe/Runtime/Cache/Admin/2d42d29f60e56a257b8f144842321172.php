...
<html>
	...
	<body>
		<div id="page-content">
			<div id="main-container container-fluid" style="margin-left:40px;">
				<div id="headshow">
					...
					<button type="button" class="btn btn-warning btn-sm" onclick="back()"><span class="glyphicon glyphicon-share-alt"></span>返回上级</button>
				...
				<div id="divTable">
					...
					<input type="hidden" value="<?php echo ($menu_id); ?>" id="menu_id">
					<input type="hidden" value="<?php echo ($parentid); ?>" id="parentid">
				</div>
			</div>
		</div>
		...
		$('#table').bootstrapTable({
					...
					method: 'get',
					url: "/html/webframework/webframe/index.php/Admin/Menu/childAjaxIndex",
					//cache: false,
			...
			function queryParams(params) {
				...
				if(typeof(params.menu_id)=="undefined"){
			...
			*/
			function add(){
				...
				var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['添加子菜单', 'font-size:14px;background:#2b9af6;color:#fff'],
						...
						content: ['/html/webframework/webframe/index.php/Admin/Menu/addChild/menu_id/'+menu_id, 'no'],
						btn: ['确定', '取消']
						, yes: function (index) {
						...
						var obj = layer.getChildFrame('#wt-forms', index); //获取form的值
								$.ajax({
									type: 'post',
									url: '/html/webframework/webframe/index.php/Admin/Menu/insert',
									data: obj.serialize(),
									cache: false,
			...
			*/
			function back(){
				...
				window.location.href="/html/webframework/webframe/index.php/Admin/Menu/back/parentid/"+parentid;
			}
			...
			*/
			function getchild(id){
				window.location.href="/html/webframework/webframe/index.php/Admin/Menu/getchild/id/"+id;
			}
			...
			*/
			function edit(menu_id){
				var index = layer.open({
							type: 2,
							skin: 'demo-class',
							title: ['修改子菜单', 'font-size:14px;background:#2b9af6;color:#fff'],
							move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
							area: ['500px', '430px'], //设置弹出框的宽高
							shade: [0.5, '#000'], //配置遮罩层颜色和透明度
							shadeClose: true, //是否允许点击遮罩层关闭弹窗 true /false
							...
							// time:1000,  设置自动关闭窗口时间 1秒=1000；
							shift: 0, //打开效果：0-6 。0放大，1从上到下，2下到上，3左到右放大，4翻滚效果；5渐变；6抖窗口
							content: ['/html/webframework/webframe/index.php/Admin/Menu/editChild/menu_id/'+menu_id, 'no'],
							btn: ['确定', '取消']
							, yes: function (index) {
							...
							var obj = layer.getChildFrame('#wt-forms', index); //获取form的值
									$.ajax({
										type: 'post',
										url: '/html/webframework/webframe/index.php/Admin/Menu/update',
										data: obj.serialize(),
										cache: false,
										success: function (data) {
											if (data.status) {
			...
			*/
			function delmore(menu_id){
				layer.confirm('确定要删除该菜单以及其子菜单吗？', {
						...
						var actionUrl = "/html/webframework/webframe/index.php/Admin/Menu/del";
							$.ajax({
							type: 'post',
									url: actionUrl,
									data: {ids:ids},
